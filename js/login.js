...
var checkValue = function() {
  if(getId('login').value == "" && getId('pass').value == "") {
    ...
    if(getId('login').value != "") {
      if(getId('pass').value != "") {
        getId('loading').style.visibility = 'visible';

        var params = 'login=' + encodeURIComponent(getId('login').value) + '&pass=' + encodeURIComponent(getId('pass').value);
...
var ajaxRequest = function(params) {
  ...
  xhr.onprogress = function(event) {
    ...
    if(getId('error-log').style.visibility == 'visible') {
      getId('error-log').style.visibility == 'hidden';
    }
  ...
  xhr.onreadystatechange = function() {
    ...
    if (xhr.status != 200) {
      ...
      if(xhr.responseText == 'true') {
        window.location.href = '../index/messages.new.php';
      } else {
